...
//		}
	...
	template<typename map> 
	void KineticsMap_CHEMKIN<map>::FittedReverseKineticConstants(OpenSMOKEVectorDouble& x_bath, Eigen::MatrixXd& fittedKineticParameters)
	{			
		OpenSMOKEVectorDouble temperatures(11); 
		    ...
		    std::cout << "   assembling X and XT matrices..." << std::endl;
		{
			...
			for(int i=0;i<temperatures.Size();i++)
			{
				...
				X(i,0) = 1.;
				X(i,1) = std::log(T);
				X(i,2) = -1./PhysicalConstants::R_J_kmol/T;
			}
		...
		fittedKineticParameters = XTX.fullPivLu().solve(Y);
	}
	...
	void KineticsMap_CHEMKIN<map>::FittedReverseKineticConstants(const unsigned int k, std::ostream& fOut, Eigen::MatrixXd& fittedKineticParameters)
	{
		if (isThermodynamicallyReversible_[k] != 0)
		{
			...
			fOut << std::setw(10) << std::right << std::fixed << std::setprecision(3) << fittedKineticParameters(1,j-1);
			fOut << std::setw(16) << std::right << std::fixed << std::setprecision(2) << fittedKineticParameters(2,j-1)/Conversions::J_from_kcal;
			fOut << std::setw(5)  << "";
		}
		else if (isExplicitlyReversible_[k] != 0)
		{
		...
		switch(falloff_reaction_type_[k])
		{
			case PhysicalConstants::REACTION_TROE_FALLOFF:
				...
				if (Pr>1e-100)
				{
					...
					double nTroePlus = 0.75-1.27*logFcent_falloff_[k];
					double cTroePlus = -0.4-0.67*logFcent_falloff_[k];
					double sTroePlus = std::log10(PrPlus) + cTroePlus;
					double FPlus = std::pow(10., logFcent_falloff_[k]/(1. + boost::math::pow<2>(sTroePlus/(nTroePlus-0.14*sTroePlus))));
