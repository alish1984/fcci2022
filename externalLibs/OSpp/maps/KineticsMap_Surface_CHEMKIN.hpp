				...
				for(unsigned int s=1;s<=number_of_coverage_dependent_reactions_;s++)
				{
					...
					for(unsigned int k=0;k<coverage_dependent_species_site_type_[s-1].size();k++)
					{
						...
						correction +=	PhysicalConstants::ln_10*coverage_dependent_eta_[s-1][k]*value + 
										...
										coverage_dependent_epsilon_[s-1][k]*value/(PhysicalConstants::R_cal_mol*this->T_);
...
//		}
	...
	*/
	...
	template<typename map> 
	void KineticsMap_Surface_CHEMKIN<map>::FittedReverseKineticConstants(OpenSMOKEVectorDouble& x_bath, Eigen::MatrixXd& fittedKineticParameters)
	{			
