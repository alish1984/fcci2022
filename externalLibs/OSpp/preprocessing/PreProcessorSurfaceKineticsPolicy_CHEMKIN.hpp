...
#include "thermo/AtomicCompositionTable.h"
		...
		for(unsigned int k=0;k<number_of_materials_;k++)
		{
			...
			PhysicalConstants::UNITS_REACTION_COMPOSITION composition_units = PhysicalConstants::UNITS_STD;
			{
				...
				for (std::size_t i = 2; i <= n; i++)
				{
					...
					else
					{
						...
						std::cout << "Available options: CAL || CAL/MOLE || EVOL || EVOLTS || JOUL || JOULES/MOLE || KCAL || KCAL/MOLE || KELV || KELVINS || MOLEC || MOLECULES || MOLE || MOLES || ATM || BAR || DYN || DYNES || PAS || PASCALS || TOR || TORR || MWOFF || MWON || NONCON || SITE || SITEFR" << std::endl;
						return false;
					}
				}
			}
			...
			else
			{
				...
				for (unsigned int i=0; i<reactions_[k].size(); i++)
				{
					...
					{
						std::cout << "Reaction " << i+1 << " starting at line: " << myKinetics->indices_of_good_lines()[reaction_lines[k][i]] << std::endl;
						std::cout << "When UBI-QEP kinetic schemes are adopted all the reactions must be of UBI-QEP type" << std::endl;
						return false;
					}

					if (reactions_[k][i].isUBIQEP_Direct() != check_direct)
					{
						std::cout << "Reaction " << i+1 << " starting at line: " << myKinetics->indices_of_good_lines()[reaction_lines[k][i]] << std::endl;
