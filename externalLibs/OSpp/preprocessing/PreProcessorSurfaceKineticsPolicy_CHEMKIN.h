...
#ifndef OpenSMOKE_PreProcessorSurfaceKineticsPolicy_CHEMKIN_H
#define	OpenSMOKE_PreProcessorSurfaceKineticsPolicy_CHEMKIN_H

#include "thermo/InputFileCHEMKIN.h"
...
namespace OpenSMOKE
{
	...
	*/
	...
	class PreProcessorSurfaceKineticsPolicy_CHEMKIN
	{
	public:

		typedef std::vector<ReactionPolicy_Surface_CHEMKIN> vector_reactions;

		/**
		* Default constructor
		*/
		PreProcessorSurfaceKineticsPolicy_CHEMKIN();

		/**
		* Copy constructor
		*/
		PreProcessorSurfaceKineticsPolicy_CHEMKIN(const PreProcessorSurfaceKineticsPolicy_CHEMKIN& orig);

		/**
		* Default destructor
		*/
		virtual ~PreProcessorSurfaceKineticsPolicy_CHEMKIN();
		...
		*/
		const std::vector<std::string>& names_site_species() const { return names_site_species_; }

		/**
		*@brief This function returns the vector containg the list of bulk species
		*@return the vector containing the list of bulk species
		*/
		const std::vector<std::string>& names_bulk_species() const { return names_bulk_species_; }

		/**
		*@brief This function returns the vector containg the list of all the additional species
		*@return the vector containing the list of site species
		*/
		const std::vector<std::string> names_additional_species() const;
	...
	protected:
		...
		std::vector< std::vector< std::vector<std::string> > >	site_species;
		std::vector< std::vector< std::vector<std::string> > >	bulk_species;
		std::vector< std::vector< std::vector<double> > >		site_occupancy;
		std::vector< std::vector< std::vector<double> > >		bulk_density;
		std::vector< std::vector<std::string> >					names_of_sites;
		std::vector< std::vector<std::string> >					names_of_bulk_phases;
		std::vector< std::vector<double> >						site_density;

		std::vector<std::string>	names_site_species_;
		std::vector<double>			occupancy_site_species_;
		std::vector<std::string>	names_bulk_species_;
		std::vector<double>			density_bulk_species_;
		std::vector<unsigned int>	site_phase_membership_;
		std::vector<unsigned int>	bulk_phase_membership_;
		...
		// UBI-QEP
		bool iUBIQEP_;
		double ubiqep_chemisorption_heats_reference_temperature_;
		std::vector<std::string>					chemisorption_heats_main_names_;
		std::vector<double>							chemisorption_heats_temperature_coefficients_;
		std::vector<double>							chemisorption_heats_constant_coefficients_;
		std::vector< std::vector<std::string> >		chemisorption_heats_dependent_names_;
		std::vector< std::vector<double> > 			chemisorption_heats_dependent_coefficients_;
		std::vector< std::vector<unsigned int> >	chemisorption_heats_dependent_indices_;
...
#include "PreProcessorSurfaceKineticsPolicy_CHEMKIN.hpp"

#endif	/* OpenSMOKE_PreProcessorSurfaceKineticsPolicy_CHEMKIN_H */
