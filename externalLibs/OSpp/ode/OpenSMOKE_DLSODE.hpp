...
{
	template <typename T>
	OpenSMOKE_DLSODE<T>::OpenSMOKE_DLSODE(T* odeSystem)
	{	
	...
	template <typename T>
	void OpenSMOKE_DLSODE<T>::SetDimensions(const int n)
	{
		MemoryAllocation(n);
	}

	template <typename T>
	void OpenSMOKE_DLSODE<T>::MemoryAllocation(const int n)
	{
		istate_ = 1;					// This is the first call for a problem	
	...
	template <typename T>
	void OpenSMOKE_DLSODE<T>::AnalyzeUserOptions()
	{
		if (this->iSetMaximumOrder_)		iwork_[4] = this->maximumOrder_;
		if (this->iSetMaximumNumberOfSteps_)	iwork_[5] = this->maximumNumberOfSteps_;
		if (this->iSetMaximumNumberOfPrints_) 	iwork_[6] = this->maximumNumberOfPrints_;

		if (this->iSetFirstStep_)		rwork_[4] = this->firstStep_;
		if (this->iSetMaximumStep_)		rwork_[5] = this->maximumStep_;
		if (this->iSetMinimumStep_)		rwork_[6] = this->minimumStep_;
	}

	template <typename T>
	void OpenSMOKE_DLSODE<T>::Solve(const double xend)
	{
		istate_ = 1;					// This is the first call for a problem	
		rwork_[0] = xend;
		...
		DLSODE( this->odeSystem_->GetSystemFunctionsStatic, &this->n_, this->y_, &this->x_, &this->xend_, 
				&this->iTolerance_, this->relTolerance_, this->absTolerance_,
				&itask_, &istate_, &iOptions_, rwork_, &lrw_, iwork_, &liw_, 
				this->odeSystem_->GetAnalyticalJacobianStatic, &mf_,
				this->odeSystem_->GetWriteFunctionStatic);
		#else
		dlsode_( this->odeSystem_->GetSystemFunctionsStatic, &this->n_, this->y_, &this->x_, &this->xend_, 
				&this->iTolerance_, this->relTolerance_, this->absTolerance_,
				&itask_, &istate_, &iOptions_, rwork_, &lrw_, iwork_, &liw_, 
				this->odeSystem_->GetAnalyticalJacobianStatic, &mf_,
				this->odeSystem_->GetWriteFunctionStatic);
	...
	template <typename T>
	void OpenSMOKE_DLSODE<T>::Status() const
	{
		std::cout << "DLSODE Status:                      " << std::endl;				// Status
	...
	template <typename T>
	std::string OpenSMOKE_DLSODE<T>::Tag() const
	{
		return "DLSODE";
	}

	template <typename T>
	int OpenSMOKE_DLSODE<T>::GetNumberOfSteps() const
	{
		return iwork_[10];
	}

	template <typename T>
	int OpenSMOKE_DLSODE<T>::GetNumberOfFunctionEvaluations() const
	{
		return iwork_[11];
	}

	template <typename T>
	int OpenSMOKE_DLSODE<T>::GetNumberOfJacobianEvaluations() const
	{
		return iwork_[12];
	}

	template <typename T>
	int OpenSMOKE_DLSODE<T>:: GetNumberOfLUFactorizations() const
	{
		return iwork_[12];
	}

	template <typename T>
	int OpenSMOKE_DLSODE<T>::GetNumberOfNonLinearIterations() const
	{
		return 0;
	}

	template <typename T>
	int OpenSMOKE_DLSODE<T>::GetLastOrderUsed() const
	{
		return iwork_[13];
	}

	template <typename T>
	double OpenSMOKE_DLSODE<T>::GetLastStepUsed() const
	{
		return rwork_[10];
	}

	template <typename T>
	int OpenSMOKE_DLSODE<T>::GetNumberOfConvergenceFailures() const
	{
		return 0;
	}

	template <typename T>
	int OpenSMOKE_DLSODE<T>::GetNumberOfErrorTestFailures() const
	{
		return 0;
	}

	template <typename T>
	OpenSMOKE_DLSODE<T>::~OpenSMOKE_DLSODE(void)
	{
		delete[] this->y0_;
		delete[] this->y_;
		delete[] rwork_;
