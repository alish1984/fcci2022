...
namespace OpenSMOKE
{
	...
	void OpenSMOKE_RADAU<T>::AnalyzeUserOptions()
	{
		...
		/*
		TODO
		if (iSetMaximumNumberOfSteps_)	iwork_[5] = maximumNumberOfSteps_;
		if (iSetMaximumNumberOfPrints_) iwork_[6] = maximumNumberOfPrints_;
		if (iSetMaximumOrder_)			iwork_[8] = maximumOrder_;

		if (iSetFirstStep_)				rwork_[4] = firstStep_;
		if (iSetMaximumStep_)			rwork_[5] = maximumStep_;
		if (iSetMinimumStep_)			rwork_[6] = minimumStep_;
		*/
	}
