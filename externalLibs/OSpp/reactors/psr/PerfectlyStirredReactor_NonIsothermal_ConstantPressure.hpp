...
namespace OpenSMOKE
{
	...
	psr_options_(psr_options)
	{
		...
		thermodynamicsMap_.MolecularWeight_From_MassFractions(MWInlet_, omegaInlet_);
		thermodynamicsMap_.MoleFractions_From_MassFractions(xInlet_, MWInlet_, omegaInlet_);
		...
		thermodynamicsMap_.hMolar_Mixture_From_MoleFractions(HInlet_, xInlet_);
		thermodynamicsMap_.uMolar_Mixture_From_MoleFractions(UInlet_, xInlet_);
		...
		// Open all files
		OpenAllFiles(thermodynamicsMap_, psr_options_);
	}
	...
	int PerfectlyStirredReactor_NonIsothermal_ConstantPressure::Equations(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y, OpenSMOKE::OpenSMOKEVectorDouble& dy)
	{
		...
		for (unsigned int i=1;i<=NC_;++i)	
			dy[i] = (omegaInlet_[i]-omega_[i])/tau_ + thermodynamicsMap_.MW()[i]*R_[i]/rho_;
		...
		const double HStarI = Dot(xInlet_, thermodynamicsMap_.species_h_over_RT())*(PhysicalConstants::R_J_kmol*T_);	// [J/kmol]
	...
	void PerfectlyStirredReactor_NonIsothermal_ConstantPressure::Solve(const double tf)
	{
		...
		FinalStatus(tf, thermodynamicsMap_, psr_options_);
		FinalSummary(psr_options_.output_path() / "FinalSummary.out", tf, thermodynamicsMap_, psr_options_);

		PrintSolution();

		CloseAllFiles(psr_options_);
	}
	...
	void PerfectlyStirredReactor_NonIsothermal_ConstantPressure::SensitivityAnalysis(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y)
	{
		...
		for(unsigned int j=1;j<=NC_;j++)
			scaling_Jp[j] = thermodynamicsMap_.MW()[j]/rho_;
