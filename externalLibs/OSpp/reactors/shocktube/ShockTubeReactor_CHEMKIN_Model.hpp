...
namespace OpenSMOKE
{
	...
	ShockTubeReactor_CHEMKIN_Model::ShockTubeReactor_CHEMKIN_Model(	
								...
								OpenSMOKE::ShockTubeReactor_Options& batch_options,
	...
	shocktube_options_(batch_options)
	{
		lm_ = lm;
		rho1_ = rho1;
		...
		OpenAllFiles(thermodynamicsMap_, shocktube_options_);
	}
	...
	void ShockTubeReactor_CHEMKIN_Model::Solve(const double tf)
	{
		...
		if (shocktube_options_.verbose_output() == true)
		{
			...
			FinalStatus(tf, thermodynamicsMap_, shocktube_options_);
			FinalSummary(shocktube_options_.output_path() / "FinalSummary.out", tf, thermodynamicsMap_);
		}

		CloseAllFiles(shocktube_options_);
	}
