...
namespace OpenSMOKE
{
	...
	plugflow_options_(plugflow_options)
	{
		...
		OpenAllFiles(thermodynamicsMap_, plugflow_options_);
	}
	...
	int PlugFlowReactor_NonIsothermal::Equations(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y, OpenSMOKE::OpenSMOKEVectorDouble& dy)
	{
		...
		for (unsigned int i=1;i<=NC_;++i)	
			dy[i] = thermodynamicsMap_.MW()[i]*R_[i]/rho_/v_;
	...
	void PlugFlowReactor_NonIsothermal::Solve(const double tf)
	{
		...
		FinalStatus(tf, thermodynamicsMap_, plugflow_options_);
		FinalSummary(plugflow_options_.output_path() / "FinalSummary.out", tf, thermodynamicsMap_, plugflow_options_);
	...
	void PlugFlowReactor_NonIsothermal::SensitivityAnalysis(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y)
	{
		...
		else
		{
			...
			for(unsigned int j=1;j<=NC_;j++)
				scaling_Jp[j] = thermodynamicsMap_.MW()[j]/rho_/v_;
