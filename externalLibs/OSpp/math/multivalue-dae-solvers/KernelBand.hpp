...
namespace DaeSMOKE
{
	...
	KernelBand<DAESystemObject>::KernelBand()
	{
	}

	template <typename DAESystemObject>
	void KernelBand<DAESystemObject>::ResetKernel()
	{
	...
	template <typename DAESystemObject>
	void KernelBand<DAESystemObject>::MemoryAllocationKernel()
	{
		...
		hJ_.resize(this->ne_);
		diagonal_.resize(this->ne_);
		index_.resize(this->ne_);
	}
	...
	template <typename DAESystemObject>
	void KernelBand<DAESystemObject>::SetTridiagonalBlockSize(const unsigned int block_size)
	{
	...
	template <typename DAESystemObject>
	void KernelBand<DAESystemObject>::SetBandSizes(const unsigned int nUpper, const unsigned int nLower)
	{
	...
	template <typename DAESystemObject>
	void KernelBand<DAESystemObject>::PreProcessing()
	{
	...
	template <typename DAESystemObject>
	void KernelBand<DAESystemObject>::SetLinearAlgebraSolver(const std::string linear_algebra_solver)
	{
		if (linear_algebra_solver == "Eigen")
		{
			...
			OpenSMOKE::ErrorMessage("KernelBand<DAESystemObject>", "Requested linear algebra is not supported!");
	}

	template <typename DAESystemObject>
	void KernelBand<DAESystemObject>::JacobianTimesVector(const Eigen::VectorXd& v_in, Eigen::VectorXd* v_out)
	{	
		J_->Product(v_in.data(), v_out->data());
	}

	template <typename DAESystemObject>
	void KernelBand<DAESystemObject>::UserDefinedJacobian(const Eigen::VectorXd& y, const double t)
	{
		const double tstart = OpenSMOKE::OpenSMOKEGetCpuTime();

		// TODO
		// this->Jacobian(y, t, J_);
		OpenSMOKE::ErrorMessage("KernelBand<DAESystemObject>", "User defined Jacobian is still not supported!");
	...
	template <typename DAESystemObject>
	void KernelBand<DAESystemObject>::NumericalJacobian(Eigen::VectorXd& y, const double t, const Eigen::VectorXd& f, const double h, const Eigen::VectorXd& e,
	...
	template <typename DAESystemObject>
	void KernelBand<DAESystemObject>::BuildAndFactorizeMatrixG(const double hr0, const double rr0)
	{	
		// 1. Assembling the G matrix
		// ----------------------------------------------------------------------------------------
		J_->CopyTo(G_);									// G_ = J_
		G_->Scale(-hr0, -rr0, index_.data());			// G_ = -hr0*J_
		G_->AddDiagonal(diagonal_.data());				// G_ = I - hr0J_
	...
	template <typename DAESystemObject>
	void KernelBand<DAESystemObject>::SolveLinearSystem(Eigen::VectorXd& db)
	{
		const double tstart = OpenSMOKE::OpenSMOKEGetCpuTime();
	...
	template <typename DAESystemObject>
	void KernelBand<DAESystemObject>::DaeSolverKernelSummary(std::ostream& out)
	{
		...
		out << std::endl;
		out << "Data for the Band DAE solver Kernel" << std::endl;
		out << "---------------------------------------------------------------------------------------------------------" << std::endl;
