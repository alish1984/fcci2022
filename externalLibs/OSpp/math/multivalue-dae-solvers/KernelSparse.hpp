...
namespace DaeSMOKE
{
	...
	template <typename DAESystemObject>
	void KernelSparse<DAESystemObject>::SetSparsityPattern(const std::vector<unsigned int>& i, const std::vector<unsigned int>& j)
	{
		i_ = i;
		j_ = j;
	}
	...
	template <typename DAESystemObject>
	void KernelSparse<DAESystemObject>::BuildAndFactorizeMatrixG(const double hr0, const double rr0)
	{
		// Assembling matrix G
		G_ = J_;
	...
	template <typename DAESystemObject>
	void KernelSparse<DAESystemObject>::SolveLinearSystem(Eigen::VectorXd& db)
	{
		const double tstart = OpenSMOKE::OpenSMOKEGetCpuTime();
	...
	template <typename DAESystemObject>
	void KernelSparse<DAESystemObject>::OdeSolverKernelSummary(std::ostream& out)
	{
